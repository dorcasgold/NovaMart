  ...
  return (
    ...
    <div className="group relative text-center flex flex-col gap-2 ">
      <div className="w-full h-64 md:h-80 lg:h-72 cursor-pointer relative overflow-hidden">
        <img src={imageUrl} className="w-full h-full object-cover group-hover:scale-110 duration-500" alt="productImg" loading="lazy" onClick={handleDetails} />
        <div className="absolute gap-2 inset-0 text-sm flex flex-col items-center justify-center transform translate-x-full group-hover:translate-x-16 group-hover:translate-y-16 transition-transform duration-500">
          <p onClick={() => toast.success(`${product.title} added to cart`)} className="px-3 font-semibold hover:transition-transform duration-500 py-2 bg-rose-500 text-white rounded mb-2 hover:bg-black">Add To Cart</p>
          <p className="px-3 font-semibold hover:transition-transform duration-500 py-2 bg-green-500 text-white rounded hover:bg-black" onClick={handleDetails}>View Detail</p>
        </div>
      </div>
      <div className="border-gray-300 border-[1px]">
        <div className="flex flex-col gap-2">
          <div className="stars text-red text-2xl font-semibold">{stars}</div>
          <h2 className=" font-semibold text-lg">{product.title.length > 25 ? (product.title.substring(0, 25)) + '...' : product.title}</h2>
        </div>
      ...
      <div className="absolute top-4 left-0 -rotate-12">
        <p className="bg-red text-xs text-white px-4 rounded-md py-2 font-semibold">{product.availabilityStatus}</p>
      </div>
